...
const UserCard = ({singleUser, setReload = f => f, reload = undefined}) => {
  ...
  const updateBtn = () => {
    return (
      <button className="btn btn-danger btn-sm rounded-2" type="button" data-toggle="tooltip" data-placement="top" title="Update" onClick={handleUpdate}>
  ...
  const deleteBtn = () => {
    return (
      <button className="btn btn-danger btn-sm rounded-2" type="button" data-toggle="tooltip"
               data-placement="top" title="Delete" onClick={handleDeleteUser}
      >
  ...
  const getARedirect = redirect => {
    if ( redirectAllMeal ) {
      return <Redirect to={"/admin/user/allmeal/" + singleUser.username + "/" + singleUser._id} />;
    }
    if (redirectUpdate) {
      // return <Redirect to={"/user/update"} state={ id : singleUser._id} />;
      return <Redirect to={{ pathname: '/user/update', state: { id: singleUser._id } }} />;
    }
  }
  ...
  return (
        ...
        <div className="card testimonial-card" style={{color : "black"}}>
          ...
          <div className="card-body">
            ...
            <h4 className="font-weight-bold mb-4">{singleUser.firstname + " " + singleUser.lastname}</h4>
            ...
            <div className="card-header d-flex justify-content-around">
              ...
              {updateBtn()}
              <button className="btn btn-danger btn-sm rounded-2" type="button" data-toggle="tooltip" data-placement="top" title="All meals" onClick={handleAllMealBtn}>
